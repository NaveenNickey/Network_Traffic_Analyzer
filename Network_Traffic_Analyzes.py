...
import matplotlib.pyplot as plt  # If not using, remove it
import numpy as np  # If not using, remove it
...
def extract_packet_data_security(packets):
    ...
    for packet in tqdm(packets, desc="Processing packets for port scanning activity", unit="packet"):
        if IP in packet:
            ...
            # Handle TCP, UDP, and others correctly
            dst_port = 0
            if TCP in packet:
...
def detect_port_scanning(df, port_scan_threshold):
    ...
    if len(ip_addresses) > 0:
        ...
        logger.info("No potential port scanning detected.")
...
def plot_protocol_distribution(protocol_counts):
    # Plot protocol distribution (implement the graph here)
...
def plot_share_of_protocols_between_ips(ip_communication_protocols):
    # Plot share of protocols between IPs (implement the graph here)
    plt.figure(figsize=(10, 6))
    ip_communication_protocols.groupby('Protocol').size().plot(kind='pie', autopct='%1.1f%%', startangle=90)
